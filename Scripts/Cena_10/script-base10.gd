	...
	if game_end == false:
		...
		for i in $spots.get_children():
			if i.occupied:
				spots -= 1
		if spots == 0:
			var mundo_2 = get_node("/root/SaveSystem")
			mundo_2.mundo_2 = "false"
			get_node("/root/SaveSystem").saveValue("values", "valueOne", mundo_2)
			...
			save_file.disabled7 = "false"
			save_file.disabled8 = "false"
			save_file.disabled9 = "false"
			
			save_file.contador_mundo1 = 10
