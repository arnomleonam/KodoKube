...
func _ready():
	$laser/AnimationPlayer.play("laser_1x7")
...
func _process(delta):
	...
	if game_end == false:
		...
		if spots == 0:
			...
			var disabled4 = get_node("/root/SaveSystem")
			disabled4.disabled4 = "false"
			get_node("/root/SaveSystem").saveValue("values", "valueOne", disabled4)
